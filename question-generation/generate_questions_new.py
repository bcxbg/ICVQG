...
import question_engine_new_copy as qeng
...
parser.add_argument('--metadata_file', default='metadatanew.json',
    help="JSON file containing metadata about functions")
parser.add_argument('--synonyms_json', default='synonyms.json',
    help="JSON file defining synonyms for parameter values")
parser.add_argument('--template_dir', default='2',
    help="Directory containing JSON templates for questions")
parser.add_argument('--input_scene_dir', default='scene_results_exmp',
    help="Directory containing JSON scene graph")

# Output
parser.add_argument('--output_questions_file',
    default='output/2.json',
...
parser.add_argument('--templates_per_image', default=6, type=int,
...
def precompute_filter_options(scene_struct, metadata):
  ...
  if metadata['dataset'] == 'CLEVR-v1.0':
    attr_keys = ["det","color","pattern","size","material","state","shape","backdet","label"]
    #attr_keys = ['size', 'color', 'shape', 'class']
  else:
    assert False, 'Unrecognized dataset'

  # Precompute masks
  masks = []
  for i in range(2 ** (len(attr_keys)-1)):
    mask = [1]
    for j in range(len(attr_keys)-1):
      mask.append((i // (2 ** j)) % 2)
    masks.append(mask)
...
def find_relate_filter_options(object_idx, scene_struct, metadata,
  ...
  for rel in scene_struct['relationships']:
    if rel['predicate'] in tstruct:
      tstruct[rel['predicate']][rel['object_idx']].append(rel['subject_idx'])
    else:
      tstruct[rel['predicate']] = []
      for i in range(len(scene_struct['objects'])):
        tstruct[rel['predicate']].append([])
      tstruct[rel['predicate']][rel['object_idx']].append(rel['subject_idx'])
  #print(tstruct)
  # print("****************************************************")
  for relationship in tstruct:
    related = set(tstruct[relationship][object_idx])
    # print("@@@",relationship,related)
...
def instantiate_templates_dfs(scene_struct, now, template, metadata, answer_counts,
                              synonyms, max_instances=None, verbose=False):

  # has_goal = False
  # asking = template['nodes'][-1]['type']
  # if asking.split('_')[0]=='query':
  #   attri = asking.split('_')[1]
  #   tempobj = scene_struct[now]['objects']
  #   for to in tempobj:
  #     if not(to[attri] == None):
  #       has_goal = True
  #       break
  # else:
  #   has_goal = True
  ...
  while states:
    ...
    q = {'nodes': state['nodes']}
    outputs = qeng.answer_question(q, metadata, scene_struct[now], all_outputs=True)
    answer = outputs[-1]
    # print('outputs:')
    # print(outputs)
    ...
    for constraint in template['constraints']:
      if constraint['type'] == 'NEQ':
        ...
        if v is not None:
          skip = False
          if p_type == 'Label' and v != 'thing': skip = True
          if p_type != 'Label' and v != '': skip = True
          if skip:
            if verbose:
    ...
    if state['next_template_node'] == len(template['nodes']):
      ...
      # print(answer)
      if answer == None:
      # if (answer == 0) or (answer == False) or (answer == None):
        # print("no answer 88")
        continue
      # if (answer == 0) or (answer == False):
      #   continue
      # Use our rejection sampling heuristics to decide whether we should
      # keep this template instantiation
      
      if answer not in answer_counts.keys():
      ...
      # has_relate = any(n['type'] == 'relate' for n in q['nodes'])
      if has_relate:
        degen = qeng.is_degenerate(q, metadata, scene_struct[now], answer=answer,
                                   verbose=verbose)
        if degen:
          # print("feihua 88")
          continue
    ...
    if next_node['type'] in special_nodes:
      if next_node['type'].startswith('relate_filter'):
        ...
        filter_options = find_relate_filter_options(answer, scene_struct[now], metadata,
                            unique=unique, include_zero=include_zero)
      else:
        filter_options = find_filter_options(answer, scene_struct[now], metadata)
        if next_node['type'] == 'filter':
          # Remove null filter
          filter_options.pop((None, None, None, None, None, None, None, None, None), None)
      ...
      for k in filter_option_keys:
        ...
        for param_name, param_val in zip(filter_side_inputs, k):
          ...
          #print('param_name_to_type:     ',param_name_to_type)
          param_type = param_name_to_type[param_name]
          ...
          if param_val is not None:
            ...
            if metadata['dataset'] == 'CLEVR-v1.0' and param_type == 'Label':
              param_val = 'thing'
            else:
              param_val = ''
            cur_next_vals[param_name] = param_val
        ...
        if extra_type is not None:
          #print('   type:   ',extra_type)
          #print('   inputs: ',input_map[next_node['inputs'][0]] + len(new_nodes))
          new_nodes.append({
            'type': extra_type,
        ...
        #print('   !!!!!!state+!!!')
        #print('     nodes:  ',state['nodes'] + new_nodes)
        #print('     vals:  ',cur_next_vals)
        #print('     input_map:  ',input_map)
        #print('     next_template_node:  ',state['next_template_node'] + 1)
        states.append({
  ...
  if now < 4 and max_instances is not None and len(final_states) < max_instances:
    still = max_instances - len(final_states)
    t,s,a = instantiate_templates_dfs(scene_struct, now+1, template, metadata, answer_counts,
                              synonyms, max_instances=still, verbose=False)
    text_questions.extend(t)
    structured_questions.extend(s)
    answers.extend(a)
...
def main(args):
  ...
  for fn in os.listdir(args.template_dir):
    ...
    with open(os.path.join(args.template_dir, fn), 'r') as f:
      ...
      for i, template in enumerate(temppp):
  ...
  # print('template_counts:' , template_counts)
  # print('template_answer_counts:' , template_answer_counts)

  all_scenes = []
  for fn in os.listdir(args.input_scene_dir):
    if not fn.endswith('.json'):continue
    print(fn)
    with open(os.path.join(args.input_scene_dir,fn),'r') as f:
      scene_data = json.load(f)
      all_scenes.append(scene_data['scenes'])
      scene_info = scene_data['info']
  begin = args.scene_start_idx
  if args.num_scenes > 0:
    end = args.scene_start_idx + args.num_scenes
    for iii in range(0,5):
      all_scenes[iii] = all_scenes[iii][begin:end]
  else:
    for iii in range(0,5):
      all_scenes[iii] = all_scenes[iii][begin:]
...
#  print(all_scenes)
  ...
  questions = []
  scene_count = 0
  for i, scene in enumerate(all_scenes[0]):
    scene_fn = scene['image_filename']
    scene_imid = scene['image_index']
    print(scene_imid)
    scene_struct = []
    for iii in range(0,5):
      scene_struct.append(all_scenes[iii][i])
    # print('starting image %s (%d / %d)'
    #       % (scene_fn, i + 1, len(all_scenes[0])))
