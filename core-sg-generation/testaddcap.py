...
def solve():
    ...
    caption_info = json.load(open('./region_descriptions.json'))
    caption_dict = {}
    for i in caption_info:
        ...
        caption = []
        for j in i["regions"]:
            caption.append(j["phrase"])
        caption_dict[img] = caption
    print('-----------------------------id to captions-------------------------------------')
    ...
    for i,prediction in enumerate(predictions):
        print(i)
        img_path = detected_info[i]['img_file']
    ...
    torch.save({'groundtruths':groundtruths, 'predictions':newpres}, detected_origin_path + 'eval_results_relnew.pytorch')
