...
print('1')
detected_origin_result = torch.load(detected_origin_path + 'eval_results_new.pytorch')
print('2')
...
idx2label = {}
for i,l in enumerate(data_info['ind_to_classes']):
  idx2label[str(i)] = l
idx2attr = {}
for i,a in enumerate(data_info['ind_to_attributes']):
  idx2attr[str(i)] = a
idx2pred = {}
for i,p in enumerate(data_info['ind_to_predicates']):
  idx2pred[str(i)] = p
print('ok')
...
model.cuda()
model.eval()
save_path = detected_origin_path + 'cpd_results/3/'
...
# test1,test2,test3,test4,test5 = show_all(start_idx=0, length=10000)
# test1,test2,test3,test4,test5 = show_all(start_idx=10000, length=10000)
test1,test2,test3,test4,test5 = show_all(start_idx=20000, length=totalnum-20000)
...
scenetest_save_path = detected_origin_path + 'scene_results_all/3/'
